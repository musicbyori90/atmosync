...
// ─── HELPERS ──────────────────────────────────────────────────────────────────
const css = `@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800&family=DM+Sans:wght@300;400;500;600&display=swap'); *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; } body { background: ${T.bg}; color: ${T.text}; font-family: ${T.body}; direction: rtl; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-thumb { background: ${T.border}; border-radius: 2px; } input, select, button, textarea { font-family: ${T.body}; } input[type=range] { -webkit-appearance: none; height: 4px; border-radius: 2px; background: ${T.border}; cursor: pointer; } input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 16px; height: 16px; border-radius: 50%; cursor: pointer; } @keyframes fadeUp { from { opacity:0; transform:translateY(16px); } to { opacity:1; transform:translateY(0); } } @keyframes pulse { 0%,100% { opacity:.5; transform:scale(1); } 50% { opacity:1; transform:scale(1.04); } } @keyframes wave { 0%,100% { transform:scaleY(.3); } 50% { transform:scaleY(1.5); } } @keyframes spin { to { transform:rotate(360deg); } } @keyframes glow { 0%,100% { box-shadow:0 4px 24px ${T.accent}22; } 50% { box-shadow:0 8px 32px ${T.accent}44; } } .fade-up { animation: fadeUp .45s ease both; } .card { background:${T.card}; border:1px solid ${T.border}; border-radius:16px; box-shadow:0 2px 12px rgba(0,0,0,0.06); } .btn-p { background:linear-gradient(135deg,${T.accent},#1f7a6e); border:none; border-radius:12px; color:#fff; font-weight:600; cursor:pointer; transition:all .2s; } .btn-p:hover { transform:translateY(-1px); box-shadow:0 6px 20px ${T.accent}44; } .btn-g { background:#fff; border:1.5px solid ${T.border}; border-radius:12px; color:${T.muted}; cursor:pointer; transition:all .2s; } .btn-g:hover { border-color:${T.accent}; color:${T.accent}; }`;
